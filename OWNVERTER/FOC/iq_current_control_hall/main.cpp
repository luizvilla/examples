...
{
	Idq_ref.d = 0.0F;
	Idq_ref.q = clamp(manual_iq_ref, -IQ_REF_MAX, IQ_REF_MAX);
...
void setup_routine()
{
	...
	scope.connectChannel(electrical_angle, "angle");
	scope.connectChannel(electrical_speed, "speed");
...
void loop_background_task()
{
	...
	case 'o':
		...
		break;
	case 'u':
		manual_iq_ref = clamp(manual_iq_ref + 0.1F, -IQ_REF_MAX, IQ_REF_MAX);
		break;
	case 'd':
		manual_iq_ref = clamp(manual_iq_ref - 0.1F, -IQ_REF_MAX, IQ_REF_MAX);
		break;
...
{
	if (!memory_print) {
		printk("state:%d init:%u valid:%u Iq_ref:%.2f Iq_meas:%.2f angle:%.2f speed:%.2f Vhigh:%.2f\n",
		       control_state,
