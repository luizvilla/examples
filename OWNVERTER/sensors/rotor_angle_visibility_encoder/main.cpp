...
void setup_routine()
{
	...
	scope.connectChannel(encoder_count_f, "encoder_count");
	scope.connectChannel(encoder_delta_count_f, "encoder_delta_count");
...
{
	if (!memory_print) {
		printk("mode:%u init:%u valid:%u count:%lu dcount:%ld mech:%.3f elec:%.3f w_mech:%.3f w_elec:%.3f\n",
		       mode,
...
void loop_critical_task()
{
	if (stream_enabled && position_sensor_initialized && shield.position.update(Ts)) {
		...
		encoder_count_f = 0.0F;
		encoder_delta_count_f = 0.0F;
